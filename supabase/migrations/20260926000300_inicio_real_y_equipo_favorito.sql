...
returns table(game_name text, username text, hits integer, rank_position integer)
language sql stable security definer set search_path = '' as $$
  with latest as (
    ...
    select latest.name as game_name, prof.username, p.hits,
      rank() over (order by p.hits desc, p.processed_matches desc)::integer as rank_position,
  ...
  select ranked.game_name::text, coalesce(nullif(ranked.username, ''), 'Jugador')::text,
    ranked.hits, ranked.rank_position from ranked where ranked.row_number <= 3 order by ranked.row_number;
$$;
